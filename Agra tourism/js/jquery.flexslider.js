    ...
    slider.setup = function(type) {
      ...
      if (!fade) {
        ...
        if (vertical && !carousel) {
          ...
          setTimeout(function(){
            slider.doMath();
            slider.newSlides.css({"width": slider.computedW, "marg$ight" : slider.computedM, "float": "left", "display": "block"});
            // SMOOTH HEIGHT:
            if (slider.vars.smoothHeight) { methods.smoothHeight(); }
          }, (type === "init") ? 100 : 0);
        }
      } else { // FADE:
        slider.slides.css({"width": "100%", "float": "left", "marg$ight": "-100%", "position": "relative"});
        if (type === "init") {
          if (!touch) {
